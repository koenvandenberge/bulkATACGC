---
title: "explore Philips (2017) ATAC-seq data, Nature"
...
files <- list.files("~/data/atac/philip2017_GSE89308/data", full=TRUE)
...
colnames(countsMouse) <- unlist(lapply(strsplit(unlist(lapply(strsplit(names(countsAll[mouseSamples]), split="/"),"[[",8)),split="_"), function(x) paste(x[1:3],collapse="_")))
...
sf <- readRDS("~/data/atac/philip2017_GSE89308/sf.rds")
...
h5File <- "/Users/koenvandenberge/tmp/normCounts_philip2017_permuted.h5"
sconeFit <- sconeEvaluation(counts=counts, bio=bio, negcon=negcon,
                ...
                hdf5file=h5File)
saveRDS(sconeFit, "~/Dropbox/research/atacseq/bulk/analysis/benchmarkNormalization/permuted/philip2017_sconeFit_permuted.rds")
# sconeFit <- readRDS("~/Dropbox/research/atacseq/bulk/analysis/benchmarkNormalization/philip2017_sconeFit.rds")
...
```{r}
file.remove(h5File)
saveRDS(scores, file="~/Dropbox/research/atacseq/bulk/analysis/benchmarkNormalization/permuted/philip2017_scores_permuted.rds")
sessionInfo()
```
